...
// 0xc2132D05D31c914a87C6611C10748AEb04B58e8F USDT
    ...
    const apiPrices = 'https://min-api.cryptocompare.com/data/price?fsym=USD&tsyms=EUR,BRL,MATIC,ETH,BTC';

    const apiMATICusd = 'https://min-api.cryptocompare.com/data/pricemultifull?fsyms=MATIC&tsyms=USD,BRL,EUR';
    ...
    const apiUSD = 'https://min-api.cryptocompare.com/data/price?fsym=USD&tsyms=EUR,BRL';
...
let i = 0;
let shortInterv = Number(20000);
let longInterv = Number(100000);

    //setInterval(getWMATIConPool, shortInterv);
    //setInterval(getPlataOnPool, shortInterv);
    //setInterval(getEURusd, longInterv);
    //setInterval(getBRLusd, longInterv);
    //setInterval(getMATICpriceUSD, shortInterv);
    //setInterval(getMATICpriceBRL, shortInterv);
    //setInterval(getMATICpriceEUR, shortInterv);

async function getAssetPrices(){
    const response = await fetch(apiPrices);
    const dataApiPrices = await response.json();
    const { result } = dataApiPrices;
...
async function getUSD(){
    const response = await fetch(apiUSD);
    const DATAapiUSD = await response.json();
    const { EUR } = DATAapiUSD;
    _EURUSD = Number(RAW.EUR.USD.PRICE).toFixed(4);
...
async function getBRLusd(){
    ...
    _BRLUSD = Number(RAW.BRL.USD.PRICE).toFixed(4);
...
async function getEURusd(){
    ...
    _EURUSD = Number(RAW.EUR.USD.PRICE).toFixed(4);
   //console.log("BRLUSD : "  + _EURUSD);
} //getEURusd();

/*
async function getMATIClastPrice(){
    const response = await fetch(MaticLastPrice);
    const DATAmaticLastPrice = await response.json();
    const { result } = DATAmaticLastPrice;
    //_MATICusd = Number(DATAapiMATICusd.RAW.MATIC.USD.PRICE).toFixed(4);
   //console.log(result.maticusd);
} //getMATIClastPrice();*/
...
async function getMATICpriceBRL(){
    ...
    _MATICbrl = Number(DATAapiMATICbrl.RAW.MATIC.BRL.PRICE).toFixed(4);
...
async function getMATICpriceEUR(){
    ...
    _MATICeur = Number(DATAapiMATICeur.RAW.MATIC.EUR.PRICE).toFixed(4);
