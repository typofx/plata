...
<body>
    ...
    if (isset($_GET['id'])) {
        ...
        $sql = "SELECT id, date, bank, plata, amount, asset, address, txid, email, status FROM granna80_bdlinks.granna_payments WHERE id = ?";
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $sql = "UPDATE granna80_bdlinks.granna_payments SET date=?, bank=?, plata=?, amount=?, asset=?, address=?, txid=?, email=?, status=? WHERE id=?";
        $stmt = $conn->prepare($sql);
