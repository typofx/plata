...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM granna80_bdlinks.token_historical_data_eurusd WHERE id = $id";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Query to update the record (without volume and market cap)
    $sql = "UPDATE granna80_bdlinks.token_historical_data_eurusd SET 
                date = '$date', 
                price = $price
            WHERE id = $id";
...
<form method="POST" action="edit.php?id=<?= $id; ?>">
    ...
    <input type="text" id="price" name="price" value="<?= number_format($row['price'], 4, '.', ''); ?>"><br><br>
