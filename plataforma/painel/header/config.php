...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_header'])) {
    ...
    $checkQuery = "SELECT id FROM granna80_bdlinks.plata_header_config_items WHERE id = 1";
    $checkResult = mysqli_query($conn, $checkQuery);

    if (mysqli_num_rows($checkResult) > 0) {
        // Update the existing record
        $updateQuery = "UPDATE granna80_bdlinks.plata_header_config_items 
                    SET logo_image = '$logoImage', logo_link = '$logoLink', background_color = '$backgroundColor' 
                    WHERE id = 1";
    } else {
        // Insert a new record
        $updateQuery = "INSERT INTO granna80_bdlinks.plata_header_config_items 
...
$query = "SELECT logo_image, logo_link, background_color FROM granna80_bdlinks.plata_header_config_items WHERE id = 1";
