...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["column_name"])) {
    ...
    $check_query = "SELECT COUNT(*) as total FROM granna80_bdlinks.granna_footer_columns WHERE column_name = ?";
    ...
    if ($total > 0) {
        ...
        $order_query = "SELECT IFNULL(MAX(column_order), 0) + 1 FROM granna80_bdlinks.granna_footer_columns";
        $result = mysqli_query($conn, $order_query);
        $row = mysqli_fetch_array($result);
        $new_order = $row[0];

        // Insert new column into the table
        $insert_query = "INSERT INTO granna80_bdlinks.granna_footer_columns (column_name, column_order) VALUES (?, ?)";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["edit_id"]) && isset($_POST["edit_name"])) {
    ...
    $update_query = "UPDATE granna80_bdlinks.granna_footer_columns SET column_name = ? WHERE id = ?";
...
if (isset($_GET["delete"])) {
    ...
    // Delete items from granna_footer linked to this column
    $delete_items_query = "DELETE FROM granna80_bdlinks.granna_footer WHERE column_id = ?";
    $stmt = mysqli_prepare($conn, $delete_items_query);
    mysqli_stmt_bind_param($stmt, "i", $column_id);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);

    // Delete column from granna_footer_columns
    $delete_query = "DELETE FROM granna80_bdlinks.granna_footer_columns WHERE id = ?";
...
$query = "SELECT * FROM granna80_bdlinks.granna_footer_columns ORDER BY column_order";
