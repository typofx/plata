...
$sql = "SELECT * FROM granna80_bdlinks.plata_footer_editable_items LIMIT 1";
$result = $conn->query($sql);
$row = $result->fetch_assoc();

// If no record exists, insert a default one
if (!$row) {
    $sql = "INSERT INTO granna80_bdlinks.plata_footer_editable_items (folder_path, background_color, terms_conditions_link, slogan, privacy_link, year, icon_background_color)
            VALUES ('', '#ffffff', '', '', '', '', '')";
    if ($conn->query($sql)) {
        $row = [
            'folder_path' => '',
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE granna80_bdlinks.plata_footer_editable_items 
