        ...
        <form action="update.php" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="file" name="logo" id="logo" class="form-control" accept=".jpg, .jpeg, .png, .ico"
                    onchange="validateFileSize(this)"> <!-- required removed-->
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="file" name="full_logo" id="full_logo" class="form-control"
                    accept=".jpg, .jpeg, .png, .ico, .svg" onchange="validateFileSize(this)"> <!-- required removed-->
            </div>

            <div class="mb-3">
                <label for="listed" class="form-label">Listed?</label>
                <select name="listed" id="listed" class="form-select">
                    <option value="1" <?php if ($row["Listed"] == '1')
                        echo "selected"; ?>>YES</option>
                    <option value="0" <?php if ($row["Listed"] == '0')
                        echo "selected"; ?>>NO</option>
                </select>
            </div>

            <div class="mb-3">
                <label for="desktop" class="form-label">Desktop:</label> <!-- ComboBox added -->
                <select name="desktop" id="desktop" class="form-select">
                    <option value="YES" <?php echo ($row["Desktop"] == 'YES') ? 'selected' : ''; ?>>YES</option>
                    <option value="NO" <?php echo ($row["Desktop"] == 'NO') ? 'selected' : ''; ?>>NO</option>
                </select>
            </div>

            <div class="mb-3">
                <label for="mobile" class="form-label">Mobile:</label> <!-- ComboBox added -->
                <select name="mobile" id="mobile" class="form-select">
                    <option value="YES" <?php echo ($row["Mobile"] == 'YES') ? 'selected' : ''; ?>>YES</option>
                    <option value="NO" <?php echo ($row["Mobile"] == 'NO') ? 'selected' : ''; ?>>NO</option>
                </select>
            </div>
            ...
            <div class="mb-3">
                <label for="country" class="form-label">Country:</label> <!-- ComboBox populated from countries.txt -->
                <select name="country" class="form-select">
                    <?php
                    $filepath = 'countries.txt';

                    if (file_exists($filepath)) {
                        $countries = file($filepath, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
                        $DbValue = trim($row['Country'] ?? '');

                        foreach ($countries as $country) {
                            $country = trim($country);
                            $selected = ($DbValue == $country) ? "selected" : "";
                            echo "<option value='$country' $selected>$country</option>";
                        }
                    } else {
                        echo "<option>Error: countries.txt file not found.</option>";
                    }
                    ?>
                </select>
            </div>
