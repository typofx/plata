    ...
    <form id="voteForm" action="submit_vote.php" method="post" enctype="multipart/form-data" onsubmit="return get_action();">
      ...
      <div class="row">
        <div class="col-lg-6">
          ...
          <button type="button" class="btn btn-primary" onclick="selectImage()">Select Image</button>
  ...
  <script>
    ...
    document.getElementById('pasteButton').addEventListener('click', async () => {
      if (!navigator.clipboard || !navigator.clipboard.read) {
        alert("The paste image functionality is not supported in this browser.");
        return;
      }
      ...
      try {
        ...
        alert('Could not access clipboard contents.');
      }
    });
    ...
    $("body").on("change", "#browse_image", function(e) {
      ...
      if (files && files.length > 0) {
        ...
        // Update the file input with the uploaded image
        document.getElementById('vote_image').files = e.target.files;
      }
    });

    function selectImage() {
      $('#browse_image').click(); // Simulate the click on the file input to open the file explorer
    }
  </script>
</body>
